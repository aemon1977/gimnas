...
if ($result) {
    ...
    if ($column && $column['Type'] !== 'longblob') {
        ...
        if ($conn->query("ALTER TABLE esporadics MODIFY COLUMN Foto LONGBLOB") === TRUE) {
...
if (isset($_POST['eliminar_foto']) && $soci) {
    $sql_delete_foto = "UPDATE esporadics SET Foto=NULL WHERE DNI='" . $conn->real_escape_string($soci['DNI']) . "'";
